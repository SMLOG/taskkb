                  ...
                  <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 sticky top-0 z-10">
                      <tr>
                        <th v-for="column in Object.keys(columnMappings)" :key="column"
                          ...
                          :style="{ width: columnWidths[column] || '180px' }" draggable="true"
                          @dragstart="dragStartColumn($event, column)" @dragover="dragOverColumn"
                          @dragleave="dragLeaveColumn" @drop="dropColumn($event, column)"
                          ...
                          <div
                            class="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-gray-200 opacity-0 group-hover:opacity-100"
                            @mousedown="startResize($event, column)">
                          </div>

                          <div v-if="selectedColumn === column"
                            class="absolute inset-0 border-2 border-blue-400 pointer-events-none"></div>
                        </th>
                      </tr>
                    </thead>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                      <tr v-for="(item, index) in selectedList.slice(0, 100)" :key="index" class="hover:bg-gray-50">
                        <td v-for="column in Object.keys(columnMappings)" :key="column"
                          class="px-4 py-2 text-sm text-gray-700" :style="{ width: columnWidths[column] || '180px' }">
                          <div class="truncate">
                            {{
                              evaluateExpression(
