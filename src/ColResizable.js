	...
	var memento = function(t, th){ 
		...
		if(th){										//in deserialization mode (after a postback)
			...
			for(;i<t.ln;i++){						//for each column
				...
				th.eq(i).css("width", aux[i] ).css("min-width", aux[i] ); 	//each column width in % is restored
			}			
			for(i=0;i<t.ln;i++)
				t.cg.eq(i).css("width", aux[i]).css("min-width", aux[i]);	//this code is required in order to create an inline CSS rule with higher precedence than an existing CSS class in the "col" elements
		...
		for(var t in tables){
            if( tables.hasOwnProperty( t ) ) {
                ...
                if (t.f) {                  //in fixed mode
                    ...
                    for(i=0; i<t.ln; i++) t.c[i].css("width", M.round(1000*t.c[i].w/mw)/10 + "%").css("min-width", M.round(1000*t.c[i].w/mw)/10 + "%").l=true; 
